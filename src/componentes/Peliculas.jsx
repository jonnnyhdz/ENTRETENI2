...
function Peliculas() {
  ...
  const totalPeliculas = 200;

  useEffect(() => {
    fetchMediaData('movie', totalPeliculas);
  }, []);
  ...
  return (
    <div className="movie-cards">
      {mediaData.map((media) => (
        <div key={media.id} className="movie-card" onClick={() => handleMovieClick(media.id)}>
          <img src={`${BASE_IMAGE_URL}${media.poster_path}`} alt={media.title} />
          <h2>{media.title}</h2>
        </div>
      ))}
    </div>
